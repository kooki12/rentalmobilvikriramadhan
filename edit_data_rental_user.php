...
<form method="POST" action="update_data_rental_user.php">
<?php
include "koneksi.php";
$no_trx_yusri = $_GET['no_trx_vikri'];
$tampil = mysqli_query($koneksi, "SELECT * FROM tbl_rental_vikri WHERE
...
<div class="row">
  <div class="col mt-3">
    ...
    <input type="hidden" name="no_trx_vikri_tmp" value="<?=$data['no_trx_vikri']?>" class="formcontrol" id="no_trx_vikri_tmp" required>
    <input type="text" class="form-control" name="no_trx_vikri" value="<?=$data['no_trx_vikri']?>" class="formcontrol" id="no_trx_vikri"
...
while ($data = mysqli_fetch_array($tampil)) {
    echo "<option value='$data[nik_ktp_v]'>$data[nik_ktp_vikri] - $data[nama_vikri] </option>";
}
?>
        </select>
  </div>
...
?>
  ...
  <script>
    function hitungTotal() {
      ...
      if (!isNaN(lama_yusri) && !isNaN(harga_vikri)) {
        var total_bayar_yusri = lama_yusri * harga_vikri;
...
<div class="row">
  ...
  <div class="col mt-3">
    ...
    <input type="text" class="form-control" name="harga_vikri" id="vikri_yusri"
...
<div class="row">
 ...
 <div class="col mt-3" class="heading">
        <a href="tampil_data_rental_user.php" class="btn btn-warning">Kembali</a>
